...
submit.addEventListener("click", e => {
  ...
  if (!isSelected) alert("Provide an option");

  if (!address.value) alert("Provide the address");
  else {
    doors++;
    if (doors >= 150) on();
   ...
   <li>
    ...
    <span class="append-note">${
      note.value || "note here"
    }</span> <span class="spanData append-option" data-id="${data}">${data}</span> <svg class="svg-remove svg">
   ...
   `;
    ...
    newElement.addEventListener("click", function (e) {
      ...
      //  spanData.setAttribute("data-id", "presentation"); // changes data attribute to presentation
      ...
      if (optionEdit) {
        ...
        optionEdit.addEventListener("blur", function () {
          if (!editingCompleted) {
            const newOptionValue = optionEdit.textContent.trim();

            if (dataId === "spoken") {
              if (newOptionValue === "presentation") {
                // User changed to "presentation"
                presentCount++;
                countPresentation.innerHTML = presentCount;
                spanData.setAttribute("data-id", newOptionValue);
              } else if (newOptionValue === "closed") {
                // User changed to "closed"
                presentCount++;
                closedCount++;
                countPresentation.innerHTML = presentCount;
                countClosed.innerHTML = closedCount;
                ...
                alert("You can only choose 'presentation' or 'closed' ");
              }

              // Update data-id attribute
            } else if (dataId === "presentation") {
              if (newOptionValue === "closed") {
                // User changed to "closed"
                ...
                alert("You can only change to 'closed' ");
              }

              // Update data-id attribute
            } else if (
              newOptionValue !== "presentation" ||
              newOptionValue !== "closed"
            ) {
              alert("provide right value");
            }

            editingCompleted = true;
            optionEdit.contentEditable = "false"; // Disable editing when the option loses focus
